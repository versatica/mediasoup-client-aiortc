...
class Handler extends HandlerInterface_1.HandlerInterface {
    ...
    { track, encodings, codecOptions }) {
        ...
        const { playerId, nativeTrackId } = track.data;
        const kind = track.kind;
        await this._channel.request('handler.addTrack', this._internal, { playerId, kind });
        ...
        const mid = await this._channel.request('handler.getMid', this._internal, { nativeTrackId });
        ...
        track.addEventListener('@enabledchange', () => {
            ...
            if (track.enabled) {
                this._channel.notify('handler.enableTrack', this._internal, { nativeTrackId });
            }
            else {
                this._channel.notify('handler.disableTrack', this._internal, { nativeTrackId });
            }
        });
    ...
    async stopSending(localId) {
        ...
        await this._channel.request('handler.removeTrack', this._internal, { nativeTrackId });
    ...
    async replaceTrack(localId, track) {
        ...
        if (!mid)
            throw new Error('associated MID not found');
        const nativeTrackId = localId;
        const { playerId } = track.data;
        const kind = track.kind;
        await this._channel.request('handler.replaceTrack', this._internal, { nativeTrackId, playerId, kind });
