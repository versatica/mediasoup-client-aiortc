...
class Worker extends EnhancedEventEmitter_1.EnhancedEventEmitter {
        ...
        this._child.on('exit', (code, signal) => {
            ...
            if (!spawnDone) {
                ...
                if (code === 42) {
                    ...
                    this.emit('@failure', new Error(`[pid:${pid}, code:${code}, signal:${signal}]`));
                }
            }
            else {
                logger.error('worker process died unexpectedly [pid:%s, code:%s, signal:%s]', pid, code, signal);
                // Throw the error somewhere.
                throw new Error(`worker process died [pid:${pid}, code:${code}, signal:${signal}]`);
            }
        });
    ...
    createHandlerFactory() {
        ...
        return () => {
            const internal = { handlerId: v4_1.default() };
